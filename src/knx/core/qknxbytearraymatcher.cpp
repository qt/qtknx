...
/*! \class QKnxByteArrayMatcher
    \inmodule QtCore
    \brief The QKnxByteArrayMatcher class holds a sequence of bytes that
    ...
    indexIn() is faster than matching a plain QKnxByteArray with
    QKnxByteArray::indexOf() if repeated matching takes place. This
    class offers no benefit if you are doing one-off byte array
    matches.

    Create the QKnxByteArrayMatcher with the QKnxByteArray you want to
    search for. Then call indexIn() on the QKnxByteArray that you want to
    search.

    \sa QKnxByteArray, QStringMatcher
*/
...
*/
QKnxByteArrayMatcher::QKnxByteArrayMatcher()
{
    p.p = 0;
    p.l = 0;
 ...
 */
QKnxByteArrayMatcher::QKnxByteArrayMatcher(const quint8 *pattern, int length)
{
...
*/
QKnxByteArrayMatcher::QKnxByteArrayMatcher(const QKnxByteArray &pattern)
    : q_pattern(pattern)
{
...
*/
QKnxByteArrayMatcher::QKnxByteArrayMatcher(const QKnxByteArrayMatcher &other)
{
    operator=(other);
}
...
*/
QKnxByteArrayMatcher &QKnxByteArrayMatcher::operator=(const QKnxByteArrayMatcher &other)
{
...
*/
int QKnxByteArrayMatcher::indexIn(const QKnxByteArray &ba, int from) const
{
    if (from < 0)
        from = 0;
...
*/
int QKnxByteArrayMatcher::indexIn(const quint8 *str, int len, int from) const
{
    if (from < 0)
        from = 0;
...
/*!
    \fn QKnxByteArray QKnxByteArrayMatcher::pattern() const
...
static int findChar(const quint8 *str, int len, quint8 ch, int from)
{
...
static int qFindByteArrayBoyerMoore(
    const quint8 *haystack, int haystackLen, int haystackOffset,
    const quint8 *needle, int needleLen)
{
    uchar skiptable[256];
 ...
 */
int qFindByteArray(
    const quint8 *haystack0, int haystackLen, int from,
    const quint8 *needle, int needleLen)
{
    ...
    */
    const quint8 *haystack = haystack0 + from;
    const quint8 *end = haystack0 + (l - sl);
    ...
    while (haystack <= end) {
        hashHaystack += *(haystack + sl_minus_1);
        if (hashHaystack == hashNeedle && *needle == *haystack && memcmp(needle, haystack, sl) == 0)
